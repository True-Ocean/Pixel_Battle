...
- [x] 属性変更: カード詳細の属性リタッチ（300px）/ 属性セレクト（💎100）
- [x] 編集画面: 名前常時編集・保存時一括課金（リネーム **200px/回** + キャンバス拡大）
- [x] Step 3–5: 戦闘エンジン、5枚デッキ配置、CPU戦、Lost（経済連動）、戦績
- [x] 盤面UI: CPU5枚見せ合い、BattleCard（BP右上・属性左下・盾・フォーカス）、クリック操作
- [x] カードノート: プレミアム編集・全員閲覧（[`ECONOMY_SPEC §8.5`](docs/ECONOMY_SPEC.md#85-カードノート)）
- [x] 属性詳細: battleGuide 簡潔化・用語タップモーダル（[`ATTRIBUTE_SPEC §2.5`](docs/ATTRIBUTE_SPEC.md#25-カード詳細の戦い方表示ui)）
