...
| 段階 | 内容 | ステータス |
|------|------|------------|
| **1** | **オフライン対人（公開ゴーストデッキ）** — 公開デッキ一覧・**戦力補正**・Supabase 共有 | **✅ 実装済み**（[OFFLINE_PVP_SPEC](./OFFLINE_PVP_SPEC.md) / [SUPABASE_SETUP](./SUPABASE_SETUP.md)） |
| **2** | **フレンド対戦（オンライン）** — ルームコード・**px 移動のみ**（Lost なし・連続再戦） | **✅ 実装済み**（[online-pvp-spec](./online-pvp-spec.md) / [ECONOMY §13.3](./ECONOMY_SPEC.md#133-オンライン対人戦)） |
| **2b** | **真剣勝負モード** — 勝者の戦利品選択＝敗者の Lost | **将来**（§13.3） |
| **3** | リアルタイムマッチ | **将来**（§13.3 後段） |

- **勝利報酬**（CPU・オフライン対人）: 相手墓地1枚選択 → px・かけら。**デッキ取り込み・別コレクションは不採用**（§4.1・§13.1）。
- **オフライン対人**: 挑戦者側のみリアル経済（勝＝戦利品、負＝Lost）。ゴースト作者の所持カードは不変。BP 補正は履歴再戦と同じ `prepareHistoryOpponentDeck`（戦力基準）。
- **フレンド対戦（オンライン）**: 双方の所持 px を移動。Lost なし。護符は現行モードでは対象外。
- **真剣勝負（将来）**: 双方リアル。意図的ロストが増え、**護符・復活／アルバム／削除**が本領を発揮する。
- ゴースト対人が増えると px 収入・Lost が増え、創作コスト（§1.4b）の体感が CPU 戦のみより締まる方向（§10.4 参照）。

### 1.8b アカウント連携・クラウドセーブ

| 項目 | ステータス |
|------|------------|
| 初回はユーザー名のみ → 設定で任意連携。メール＋パスワード・空クラウドでは端末優先・端末1メール固定・自動同期（`player_saves`）。手動 **クラウドに保存** / **クラウドから復元** | **✅ 実装済み**（[SUPABASE_SETUP](./SUPABASE_SETUP.md) / [PROTOTYPE §4.11](./PROTOTYPE_DEVELOPMENT_SPEC.md#411-設定画面)） |
| **アカウント削除**（Edge Function `delete-account`・二段階確認・端末初期化） | **✅ 実装済み**（同上） |
| 公開デッキのメール連携前後重複防止 | **✅ 実装済み**（[OFFLINE_PVP §6.4](./OFFLINE_PVP_SPEC.md#64-メール連携と公開デッキ)） |
| Sign in with Apple / Google・日本語メール／独自 SMTP | **リリース前**（[SUPABASE_SETUP 将来実装](./SUPABASE_SETUP.md#将来実装リリース前)） |
...
| **サブスク** | ライト **500円** / プレ **800円**（月額のみ・**排他**）— [§12](./ECONOMY_SPEC.md#12-課金サブスクリプション)。ライト→プレ **日割り差額**アップグレード。月次付与は **毎月一括受取報酬** 1行表示。ライト/プレ共通で **デッキ名変更**。プレ特典に **リネーム無料**・カードノート |
...
| バトル履歴・履歴再戦 | ✅ `RecordsScreen`・`BattleHistoryList`・再戦フロー・**勝敗別 px**（`HistoryRematchRewardModal`・勝利時任意/常時2倍） |
...
| ショップ画面 | ✅ **3タブ**（💎 / アイテム / サブスク）・モック購入。サブスクは **毎月一括受取報酬** 1行・プレ説明2行（**2026-07-04**） |
| デッキ3〜 💎 解放 | ✅ `unlockDeckWithJewels()`・`canUnlockDeckSlotWithJewels`・`DeckUnlockModal`（フェーズ3） |
| 💎 不足→ショップ誘導 | **不採用**（deep link は実装しない） |
| **サブスク特典（§11.5 / §12.2）** | ✅ ライト CM 解除・プレ常時2倍・**リネーム無料**・カードノート・**デッキ名変更**（ライト/プレ。`canRenameDeck`） |
| **ミッション** | ✅ デイリー4〜5/ウィークリー7（+**全達成ボーナス** 💎2/💎5）/常設 16 カウンター+属性・R/SR トラック/ビギナー12 STEP（[PROTOTYPE §4.8](./PROTOTYPE_DEVELOPMENT_SPEC.md#48-ミッション)） |
| **BGM** | ✅ `bgmPlayer` / `soundEnabled` / 設定画面トグル（[PROTOTYPE §4.10](./PROTOTYPE_DEVELOPMENT_SPEC.md#410-サウンドbgm)） |
| **ヘルプ** | ✅ バトルハブ/エディタ/バトル履歴/マイデッキ/思い出アルバム ? モーダル・マイデッキ初回案内（[PROTOTYPE §4.9](./PROTOTYPE_DEVELOPMENT_SPEC.md#49-ヘルプ初回案内)） |
...
**完了条件**: 属性かけらで★+1が動く。各段階でBPが均等加算される。レア昇格時はかけら＋💎。汎用はショップ交換用。
...
| 真剣勝負モード / リアルタイムマッチ | **将来**（[ECONOMY §13.3](./ECONOMY_SPEC.md#133-オンライン対人戦)） |
...
20. ~~**履歴再戦報酬モーダル** — 勝敗別 px・勝利時2倍 CM~~ — **2026-07-04 完了**
...
26. ~~**クラウド同期 UI** — 手動「クラウドに保存」/「クラウドから復元」（復元は確認ダイアログ）~~ — **2026-07-05 完了**
27. ~~**アカウント削除** — Edge Function + 設定 UI（ログアウト・連携解除は UI から削除）~~ — **2026-07-05 完了**
28. ~~**公開デッキ重複防止** — メール連携・auth 切替時の orphan 行削除と再公開~~ — **2026-07-05 完了**
29. ~~**フレンド対戦（オンライン）** — ルーム・clash 再生・px 移動・再戦 UX（Lost なし）~~ — **2026-07-13 完了**（[online-pvp-spec](./online-pvp-spec.md)）
...
3. フレンド対戦の **2端末手動受け入れ**（[online-pvp-spec §8](./online-pvp-spec.md#8-受け入れ条件)）→ main へ merge
4. フェーズ **11** 残り — 真剣勝負 Lost・UR/L 等
...
- [x] ショップかけら交換: 汎用2→属性1／属性1→汎用2＋得た汎用×10px（解放済み属性・数量指定・日次上限なし）
- [x] 限界突破 BP: **基礎BP×3%/回** の均等加算（レア昇格でも減少しない）
- [x] 汎用かけらはショップ交換用（汎用2＝属性1）。限界突破は属性かけらのみ
