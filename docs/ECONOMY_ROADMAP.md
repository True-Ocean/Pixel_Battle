...
| 段階 | 内容 | ステータス |
|------|------|------------|
| **1** | **オフライン対人（公開ゴーストデッキ）** — 公開デッキ一覧から選択・挑戦側レベルで BP 補正・CPU 戦同型フロー | **方針確定・未実装**（[ECONOMY §13.2](./ECONOMY_SPEC.md#132-オフライン対人戦公開ゴーストデッキ将来構想)） |
...
| **サブスク** | ライト **500円** / プレ **800円**（月額のみ・**排他**）— [§12](./ECONOMY_SPEC.md#12-課金サブスクリプション)。ライト→プレ **日割り差額**アップグレード。月次付与は **毎月一括受取報酬** 1行表示。プレ特典に **リネーム無料**・カードノート |
...
| **サブスク特典（§11.5）** | ✅ ライト CM 解除・プレ常時2倍・**リネーム無料**・カードノート（**フェーズ9 完了**） |
