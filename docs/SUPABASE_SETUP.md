...
4. **この端末を連携**: 確認コード付きメールを送信（`signInWithOtp`。Change Email メールはコードが無いため使わない）
5. 届いたメールの **確認コード（数字）を同じアプリ画面に入力**して連携完了。リンクは開かない。確認後の auth `user_id` はメールユーザーになり、端末セーブは同期でクラウドへ上がる
6. 連携後はローカル保存が debounce 付きでクラウドへ自動同期される。設定の **今すぐ同期** でも手動実行可
7. **ログイン（復元用）**: 同様にコード入力でログイン。クラウドの方が新しければ自動復元
8. 端末ごとに連携完了したメールは固定。**連携を解除**は端末の表示・セッション解除のみで、`auth.users` は残る。同じメールで再び使うときは **ログイン（復元用）**（「この端末を連携」は未登録メール用）。別メールにする場合も解除後に新規連携。アカウントごと消すのはリリース前の「アカウント削除」
...
- 連携確認後の auth `user_id` はメールユーザー側になる（匿名 ID 固定は OTP メールにコードが載らないため採用しない）。端末セーブは localStorage に残りクラウドへ同期する
- クラウドセーブは **メール連携済み**（本ログイン）のときだけ同期する
- 同期は時刻比較に加え、**クラウドに進行が無いときは端末データを優先してアップロード**する（空アカウントでログインしても端末進行を消さない）
