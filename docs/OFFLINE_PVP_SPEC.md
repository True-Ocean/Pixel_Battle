...
| フェーズ G | **実装済み**（Supabase 公開プール・匿名認証・スロット公開トグル）。セットアップは [SUPABASE_SETUP.md](./SUPABASE_SETUP.md) |
...
| 一覧の自分 | **自分の公開デッキは出さない** |
| フォールバック | リモート失敗・未設定時も **同梱シードを一覧に含める**（リモート成功時もシードを併用） |
...
3. ~~`listPublicGhostDecks` リモート＋シード~~ — **実装済み**
