...
| 関連 | [経済仕様 §13](./ECONOMY_SPEC.md#13-対人戦) / [フレンド対戦（オンライン）](./online-pvp-spec.md) / [プロトタイプ開発指示書](./PROTOTYPE_DEVELOPMENT_SPEC.md) / [経済ロードマップ](./ECONOMY_ROADMAP.md) |
...
- 経済・戦闘ルールの正は [ECONOMY_SPEC §13](./ECONOMY_SPEC.md#13-対人戦)。本書は **実装手順・型・画面・分岐** の正とする。
...
| 挑戦側デッキ戦力での BP 補正 | フレンド対戦（オンライン）のルーム同期 |
...
|------|-------------|-------------------------------|----------|
| 入口 | バトルハブ「CPU戦」 | バトルハブ「公開デッキ戦」**（Lv10+）** | 戦績 → バトル履歴 |
...
- 「公開デッキ戦」は **Lv10 未満は disabled**（`Lv10で解放` 表示）。Lv10 以上でタップ → 公開デッキ一覧。
- 「フレンド対戦（オンライン）」も **Lv10 で解放**。実装の正は [online-pvp-spec.md](./online-pvp-spec.md)（本書の範囲外）。
...
- バトルハブヘルプ（`getBattleHubHelp`）は共通の流れ・操作・Lost を扱う。モード差分は各遷移先右上の？（`getOfflinePvpModeHelp` 等）で表示する。
- 共通: デッキ要件（5枚・ロストなし）→ 相手確定後の確認 → 準備画面（前衛2・後衛3）→ ターン操作・勝敗報酬・ロスト。
- 公開デッキ戦の差分: 公開デッキ一覧から相手を選ぶ / BP は自分のデッキ戦力に合わせて調整 / 相手の絵・属性は公開スナップショットのまま / 相手の所持カードは減らない。
- マイデッキヘルプ（`getDeckHelp`）に **公開デッキへの登録**（チェックで他ユーザーに公開）を記載。
...
- フレンド対戦（オンライン）本体（`src/onlinePvp/` — [online-pvp-spec](./online-pvp-spec.md)）
- リアルタイムマッチ・真剣勝負 Lost
...
| 項目 | 参照 |
|------|------|
| フレンド対戦（オンライン・px のみ・実装済み） | [online-pvp-spec](./online-pvp-spec.md) / [ECONOMY §13.3](./ECONOMY_SPEC.md#133-オンライン対人戦) |
| 真剣勝負（勝者選択＝敗者 Lost） | [ECONOMY §13.3](./ECONOMY_SPEC.md#133-オンライン対人戦)（将来） |
| リアルタイムマッチ | 同上 |
...
| 版 | 日付 | 内容 |
|----|------|------|
| 1.9 | 2026-07-19 | UI 用語変更 — バトルハブ入口・一覧タイトル・戦績・ミッション・レベル解放を「対人戦（オフライン）」→ **「公開デッキ戦」** に統一。デッキ公開トグルは「公開デッキに登録する／登録中」。内部識別子（`offlinePvp` 等）・DB 名は不変 |
| 1.8 | 2026-07-13 | §11.1 フレンド対戦は online-pvp-spec へ。§17 を実装済み／将来に分離 |
